...
axios.interceptors.request.use(async (config) => {
  ...
  // Add selected branch ID to request headers for multi-tenant database switching
  const selectedBranchId = localStorage.getItem("selectedBranchId");
  const isMainStore = localStorage.getItem("isMainStore");
  
  if (selectedBranchId) {
    config.headers["X-Branch-Id"] = selectedBranchId;
    // If this is the main store (All Branches), indicate overview mode
    if (isMainStore === "true") {
      config.headers["X-All-Branches"] = "true";
    }
  }
