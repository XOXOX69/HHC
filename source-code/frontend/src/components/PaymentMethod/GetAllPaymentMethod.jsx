    ...
    },
    {
      ...
      render: (methodType) => (
        <span className={`px-2 py-1 rounded text-xs font-medium ${
          methodType === 'gcash' ? 'bg-blue-100 text-blue-800' :
          methodType === 'maya' ? 'bg-green-100 text-green-800' :
          methodType === 'bank' ? 'bg-purple-100 text-purple-800' :
          methodType === 'cod' ? 'bg-orange-100 text-orange-800' :
          'bg-gray-100 text-gray-800'
        }`}>
          {methodType || 'manual'}
        </span>
      ),
      renderCsv: (methodType) => methodType || 'manual',
    },
    {
      id: 3.6,
      title: "QR Code",
      ...
      key: "qrCodeImage",
      render: (qrCodeImage) => qrCodeImage ? (
        <div className='w-[2.5rem] h-[2.5rem] relative p-1'>
          <img
            className='absolute object-cover w-full h-full rounded'
            alt='QR Code'
            onError={handleOnError}
            src={`${import.meta.env.VITE_APP_API}/files/${qrCodeImage}`}
          />
        </div>
      ) : (
