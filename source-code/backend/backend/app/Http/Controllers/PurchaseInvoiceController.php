...
class PurchaseInvoiceController extends Controller
{
    ...
    public function createSinglePurchaseInvoice(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $createdInvoice = PurchaseInvoice::create([
                ...
                'storeId' => $this->getStoreIdForCreate($request),
            ]);
    ...
    public function getAllPurchaseInvoice(Request $request): JsonResponse
    {
        ...
        if ($request->query('query') === 'info') {
            try {
                $aggregation = PurchaseInvoice::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->selectRaw('COUNT(id) as id')->first();

                // transaction of the total amount
                $totalAmount = Transaction::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->where('type', 'purchase')
                ...
                $totalPaidAmount = Transaction::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->where('type', 'purchase')
                    ->where(function ($query) {
                ...
                // transaction of the total amount
                $totalAmountOfReturn = Transaction::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                ...
                $totalInstantReturnAmount = Transaction::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->where('type', 'purchase_return')
                    ->where(function ($query) {
            ...
            try {
                ...
                $allPurchase = PurchaseInvoice::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->where(function ($q) use ($request) {
                        $q->where('id', $request->query('key'))
                ...
                $total = PurchaseInvoice::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->where(function ($q) use ($request) {
                        $q->where('id', $request->query('key'))
            ...
            try {
                $purchaseInvoices = PurchaseInvoice::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->with('purchaseInvoiceProduct', 'purchaseInvoiceProduct.product:id,name', 'supplier:id,name')
                    ->orderBy('created_at', 'desc')
            ...
            try {
                ...
                $counted = PurchaseInvoice::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                ->when($request->query('supplierId'), function ($query) use ($request) {
                    return $query->whereIn('supplierId', explode(',', $request->query('supplierId')));
