...
class TransactionController extends Controller
{
    ...
    public function getAllTransaction(Request $request): JsonResponse
    {
        ...
        if ($request->query('query') === 'info') {
            try {
                $aggregations = Transaction::when($storeId !== null, function ($q) use ($storeId) {
            ...
            try {
                $allTransaction = Transaction::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->with('debit', 'credit')->orderBy('id', 'desc')->get();
