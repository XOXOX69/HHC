...
class SaleInvoiceController extends Controller
{
    ...
    public function getAllSaleInvoice(Request $request): JsonResponse
    {
        ...
        if ($request->query('query') === 'info') {
            try {
                $aggregation = SaleInvoice::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->selectRaw('COUNT(id) as id, SUM(profit) as profit')
                    ->where('isHold', 'false')
                    ->first();
            ...
            try {
                ...
                $allSaleInvoice = SaleInvoice::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                ...
                $total = SaleInvoice::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->where('id', $request->query('key'))
                    ->count();
            ...
            try {
                ...
                $allOrder = SaleInvoice::when($storeId !== null, function ($q) use ($storeId) {
                    return $q->where('storeId', $storeId);
                })
                    ->with('saleInvoiceProduct', 'user:id,firstName,lastName,username', 'customer:id,username')
                    ->where('isHold', 'false')
                ...
                // get total count of sale invoice
                $counted = SaleInvoice::where('isHold', 'false')
                    ->count();
