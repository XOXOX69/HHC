    ...
    public function getAllProduct(Request $request): JsonResponse
    {
        ...
        if ($request->query('query') === 'all') {
            try {
                ...
                // Apply store filter if not in All Branches mode
                if ($storeId !== null) {
            ...
            try {
                $searchQuery = Product::where(function($q) use ($request) {
                    $q->where('name', 'LIKE', '%' . $request->query('key') . '%')
                      ->orWhere('sku', 'LIKE', '%' . $request->query('key') . '%');
                });
                
                // Apply store filter if not in All Branches mode
                if ($storeId !== null) {
                    $searchQuery->where('storeId', $storeId);
                }
                ...
                $totalProductcount = $searchQuery->count();
            ...
            try {
                ...
                $baseQuery = Product::where('status', 'true');
                if ($storeId !== null) {
                    $baseQuery->where('storeId', $storeId);
                }
                
                //get all product count
                $getAllProduct = (clone $baseQuery)->count();

                //get all product quantity count
                $getAllProductQuantity = (clone $baseQuery)->where('productQuantity', '>', 0)->count();

                //get all product sale price
                $getAllProductSalePrice = (clone $baseQuery)->sum('productSalePrice');

                //get all product purchase price
                $getAllProductPurchasePrice = (clone $baseQuery)->sum('productPurchasePrice');

                //get all product reorder quantity
                $getAllProductReorderQuantity = (clone $baseQuery)->where('productQuantity', '<=', DB::raw('reorderQuantity'))->count();
            ...
            try {
                ...
                $getAllProduct = Product::when($storeId !== null, function ($query) use ($storeId) {
                ...
                $totalProductcount = Product::when($storeId !== null, function ($query) use ($storeId) {
                    return $query->where('storeId', $storeId);
                })
                    ->when($request->query('productSubCategoryId'), function ($query) use ($request) {
                        return $query->whereIn('productSubCategoryId', explode(',', $request->query('productSubCategoryId')));
